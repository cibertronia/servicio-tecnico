$(function(){
	...
	if (serviProveed==1) {selectorProveedores();}
	if (serviCategoria==1) {selectorCategorias();}
	function selectorProveedores(){
		$.ajax({
			url: 'includes/consultas.php',
			type: 'POST',
			dataType: 'html',
			data: {selectorCategoriasGenerarCotizacion: ''},
			success:function(data){
				$("#selectorProveedor").html(data);
			}
		})
	}
	function selectorCategorias(){
		$.ajax({
			...
			type: 'POST',
			dataType: 'html',
			data: {selectorProveedoresGenerarCotizacion: ''},
			success:function(data){
		...
		$("#idCliente_lista option:selected").each(function() {
			...
			$.ajax({
				...
				success:function(data){
					...
					$("#ciudadCliente").val(data.idCiudad);
					$("#correoCliente").val(data.correo);
					$("#empresaCliente").val(data.empresa);
					$("#telempresaCliente").val(data.telEmpresa);
					$("#exttelempresaCliente").val(data.ext);
					$(".selectorProducto").removeClass('d-none');
					selectorProductos();
	...
	$("#selectorProducto").change(function(event) {
		$("#selectorProducto option:selected").each(function() {			
			...
			var telEmpresa 		= $("#telempresaCliente").val();
	...
	$(document).on('click', '.btn-masProductos', function(event) {
		...
		var celularCliente= $("#telefonoCliente").val();
		var ciudadCliente = $("#ciudadCliente").val();
		var telEmpresaCli	= $("#telempresaCliente").val();
		var extEmpresaCli	= $("#exttelefonoEmpresaCliente").val();
		...
		}else if (extEmpresaCli!='' & telEmpresaCli=='') {
			$(".emptyTelefonoEmpresa").removeClass('d-none');
			setTimeout(function() {
				$(".emptyTelefonoEmpresa").addClass('d-none');
				$("#telempresaCliente").focus();
	...
	$(document).on('click', '.guardarCotizacion', function(event) {
		...
		// }
			...
			garantiaDetall = (garantiaDetall == '')? ' ' : garantiaDetall;
  ...
  contador("#nombre_Cliente",90); contador("#nombreCliente",90); contador("#empresaCliente",90); contador("#empresa_Cliente",90);
});