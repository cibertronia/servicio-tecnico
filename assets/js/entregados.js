$(function () {
	...
	$(document).on("click", ".btnFacturaModalCargarDatos", function (event) {
		...
		let id = $(this).attr("id");
		$.ajax({
			url: "includes/api_facturacion/getDataSoporte.php",
			type: "POST",
			dataType: "json",
			data: { id: id },
			success: function (data) {
	...
	$(document).on('click', '.facturarSintic', function (event) {
		...
		async function clienteListo() {
			try {
				...
				let idCotizacion = $("#idCotizacion").val();//servicio sucursales id
				...
				let datosPostFactura = "customer_id=" + customer_id +
					...
					"&email=" + email +
					"&idCotizacion=" + idCotizacion +//servicio sucursales id
