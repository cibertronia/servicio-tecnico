<?php
define("DB_USER", 'suportyapa_admin');
define("DB_PASSWORD", 'ES@72900968');
define("DB_NAME", 'suportyapa_sistemav3');
define("DB_HOST", '75.119.156.102');
define("BACKUP_DIR", 'BackupSQL');
define("TABLES", 'clientes');
//define("TABLES", 'table1, table2, table3'); // Partial backup
define('IGNORE_TABLES',array('tbl_token_auth','token_auth'));
define("CHARSET", 'utf8');
...
class Backup_Database {
  ...
  public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8') {
    ...
    $this->backupFile              = TABLES.'-'.date("Ymd_His", time()).'.sql';
    ...
    try {
      ...
      foreach($tables as $table) {
        if( in_array($table, IGNORE_TABLES) )
          continue;
        //$this->obfPrint("Guardando respaldo tabla `".$table."` table...".str_repeat('.', 50-strlen($table)), 0, 0);
        /*** CREATE TABLE */
  ...
  */
  protected function gzipBackupFile($level = 9) {
    ...
    $dest =  $source . '.gz';
    //$this->obfPrint('Respaldo guardo exitosamente .... ', 1,0);
    $mode = 'wb' . $level;
